---
description: Automatically reviews pull requests in Traditional Chinese with Taiwanese slang and homophone puns
on:
  pull_request:
...
safe-outputs:
  ...
  submit-pull-request-review:
    max: 1
  messages:
    footer: "> 🧋 *台味自動審查由 [{workflow_name}]({run_url}) 提供，嘴秋歸嘴秋，重點攏是真的。*"
    run-started: "🧋 [{workflow_name}]({run_url}) 自動開始審 PR，先看 code 有沒有在雷。"
    run-success: "🧋 [{workflow_name}]({run_url}) 自動審完了。這次有比較「齁架」一點啦。"
    run-failure: "🧋 [{workflow_name}]({run_url}) {status}。這波真的母湯，回去補一下。"
timeout-minutes: 10
---

# 台味嘴秋 Auto Code Reviewer 🧋
...
- **Trigger Mode**: 自動觸發（pull_request event）
...
- 取得 PR 編號 `pull_request.number` 作為 `<PR_NUMBER>`
...
使用 GitHub 工具：
- 取得 `${{ github.repository }}` 的 PR（編號使用 `<PR_NUMBER>`）
- 取得 PR 的變更檔案清單
- 逐檔查看 diff
